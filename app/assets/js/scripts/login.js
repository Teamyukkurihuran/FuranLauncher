...
ipcRenderer.on('MSALoginWindowReply', (event, ...args) => {
    if (args[0] === 'error') {
        ...
        switch (args[1]){
            ...
            case 'AuthNotFinished': {
                setOverlayContent('ERROR', 'FuranLauncherを使用するには、ログインが必要です。ログインに成功すると、ウィンドウは自動的に閉じます。', 'OK')
                setOverlayHandler(() => {
                    toggleOverlay(false)
                    toggleOverlay(false, false, 'msOverlay')
                })
    ...
    if (queryMap.has('error')) {
        ...
        if(error === 'access_denied'){
            error = 'ERRPR'
            errorDesc = 'To use the Furan Launcher, you must agree to the required permissions! Otherwise you can\'t use this launcher with Microsoft accounts.<br><br>Despite agreeing to the permissions you don\'t give us the possibility to do anything with your account, because all data will always be sent back to you (the launcher) IMMEDIATELY and WITHOUT WAY.'
        }        
